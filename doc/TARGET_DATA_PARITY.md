...
| Excluded scanlators | `TargetExcludedScanlatorEntity` | **Covered in isolated target fixture** | Preserve trimmed, de-duplicated series policy state and reject blank/duplicate backup rows; production repository/UI cutover remains open. |
...
| Downloads | `DownloadArtifactIndex` + `DownloadArtifactVerifier` + `UniFileDownloadArtifactProbe` | **Contract, pure verifier, and UniFile observation covered; implementation incomplete** | Filesystem remains authoritative. The adapter counts supported page files and archive entries read-only; transactional index publication and legacy read-only adoption remain open. |
| Source lifecycle | `TargetSourceEntity` + `TargetSourceRepository` + `ReconcileSourceRegistry` | **Covered in isolated target store** | Real native descriptors reconcile with monotonic revision checks while preserving install/enabled state. Missing registry entries are reported, not auto-uninstalled. Credentials, permission grants, and trust decisions remain in secure storage and are excluded from backup. Production startup wiring remains blocked by target cutover. |
...
2. Download artifact verification and UniFile observation are now implemented, but transactional index publication and the legacy read-only adoption pass remain unimplemented.
3. Target source lifecycle persistence and descriptor reconciliation exist only in the isolated target store; product source management, secure credential/permission adapters, and production startup wiring remain unwired.
4. Multi-source aggregation is implemented and tested only in the isolated target read projection; production navigation and explicit source selection remain unimplemented.
5. Production backup still emits and restores legacy models.
6. No production cutover rehearsal exists.

The target-only rehearsal is implemented in `core/data/src/test/java/ephyra/data/room/target/TargetMigrationRehearsalTest.kt` and the target backup contract in `core/data/src/main/java/ephyra/data/backup/target/TargetBackupMapper.kt`. These tests prove same-title isolation, source identity, library membership, category definitions/membership, chapter state/history, target tracking, excluded-scanlator policy, protobuf round-trip, referential validation, and idempotent migration replay. Download artifact semantics, the pure verifier, and the UniFile observation adapter are covered by `DownloadArtifactIndexContractTest` and the core:download compile/archive gate; the verifier classifies missing, invalid, added, updated, and unchanged probes without mutating storage or queue state, while the adapter performs read-only directory/CBZ observation. A future index adapter will translate missing probes into Removed index rows; transactional publication and legacy adoption remain open. Source lifecycle and descriptor reconciliation are tested in `SourceLifecycleTest`, `ReconcileSourceRegistryTest`, and `TargetSourceRepositoryTest`; reconciliation preserves user install/enabled state and reports missing registry entries without deleting them. These isolated paths do not claim production parity for downloads, production source startup, tracker scheduling/adapters, or production backup restore; those remain explicit blockers.
