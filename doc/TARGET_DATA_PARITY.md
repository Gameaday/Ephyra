...
| Source lifecycle | `TargetSourceEntity` + `TargetSourceRepository` | **Covered in isolated target store** | Descriptors and install/enabled state survive Room; credentials, permission grants, and trust decisions remain in secure storage and are excluded from backup. Startup must reconcile real `SourceDescriptor` values; legacy numeric source rows are not fabricated into target lifecycle records. |
...
3. Target source lifecycle persistence exists only in the isolated target store; startup descriptor reconciliation, product source management, and secure credential/permission adapters remain unwired.
