...
| Downloads | `DownloadArtifactIndex` + `DownloadArtifactVerifier` | **Contract and pure verifier covered; implementation incomplete** | Filesystem remains authoritative. The verifier classifies missing, invalid, added, updated, and unchanged probes; a UniFile probe adapter, transactional index publication, and legacy read-only adoption remain open. |
...
2. Download artifact verification is now a pure domain boundary, but the UniFile probe adapter, transactional index publication, and legacy read-only adoption pass remain unimplemented.
