...
- `DATA-001I`: production parity matrix and migration rehearsal fixture. **In progress**; category, source-neutral tracking, excluded-scanlator policy, target backup, confirmed-link aggregate projection, isolated migration rehearsal, and source lifecycle persistence are covered. Lifecycle rows are discovered from real descriptors rather than fabricated from legacy numeric source rows; credentials/permission grants remain outside Room and backup. The [download artifact contract](../DOWNLOAD_ARTIFACT_CONTRACT.md) defines filesystem authority and queue separation, but the verifier/reconciled index remain unimplemented. Startup descriptor reconciliation, production tracker scheduling/adapters, production backup bridge, source selection/navigation, and production cutover rehearsal remain open. See `../TARGET_DATA_PARITY.md`.
