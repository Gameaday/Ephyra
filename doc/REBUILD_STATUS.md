...
| B-024 | **`DEF-001`'s final link cannot be verified in the JVM harness, and that is a named blocker rather than a pass.** Two independent reasons, both established by experiment rather than assumption. First, `graphicsLayer` is a render-only transform: it does not change layout, so `SemanticsNode.size` is constant whether or not the layer is applied, and a layout assertion cannot observe it at all. Second, `captureToImage` under Robolectric does not rasterise the `graphicsLayer` transform — proven with a control test that applied a **static** `scaleX = 2f` with no gesture involved and got an identical pixel count to the unscaled page. Pixel tests asserting a working transform were written, failed, and were **deleted rather than weakened**, because an assertion that cannot fail is a fabricated pass. Proven: real pointer events reach the real adapter and arbiter and reduce into a different transform. Not proven: that the transform reaches the screen. That link needs `E3` instrumentation. | `PagerViewportWiringTest` (5) | E2 partial, `E3` BLOCKED |
...
| RDR-004 | Paged reader replacement. | Agent | `PagerViewport` state owner + `PagerViewportTest` (12, E2), plus the Compose adapter (`PagerViewportAdapter`, `PagerViewportReducer`) and `PagerViewportWiringTest` (5, E2). The state owner is falsification-verified: reintroducing the `DEF-001` pattern turns 2 of the 12 red. The wiring test proves real pointer events reach the real adapter, the real arbiter, and reduce into a different transform. **`DEF-001`'s final link is NOT proven and is recorded as a blocker** — see `B-024`. Not production-wired; `ZoomableMangaPage` keeps its detector until the replacement owns the gesture, per rule 7. | CODE_COMPLETE at E2, not production-wired; `E3` outstanding |
...
| 2Quality infrastructure | NOT_STARTED | TST-001A, TST-001B, TST-001C, TST-002, TST-003 |
| 3Foundations | IN_PROGRESS | `ARC-001` is `CODE_COMPLETE` at `E2` (6 graph rules). `ARC-003` is `IN_PROGRESS`: the base effect/state pattern and a reference implementation exist at `E2` (22 tests), but no production feature is migrated onto it yet, which is the second half of the task. `ARC-002` is `NOT_STARTED`. Not VERIFIED. |
...
| 7Paged reader | IN_PROGRESS | RDR-004 not started, but the DEF-001 zoom defect is now fixed and production-wired: the transform is applied via graphicsLayer and anchored on the gesture centroid. E4 pending. |
| 8Continuous reader | IN_PROGRESS | RDR-005 not started, but DEF-002 and DEF-003 are now fixed and production-wired: one isotropic document transform on the scroll container, focal-anchored, replacing the per-item X-only layer. E4 pending. |
