...
| OPS-003 | Deterministic health gates that measure product properties. | Agent | `ModuleDependencyGraphTest` (6 layering rules), `SigningSecretTest` (4), `ManifestPrivilegeTest` (4), plus domain-purity and no-`data.*`-in-features CI greps. Gates are structural only: a count is always satisfiable by editing the number, so size metrics (module count, dependency edges, source and test file counts, TODO and `@Deprecated` markers) are measured on a schedule and recorded in [`BUILD_HEALTH.md`](BUILD_HEALTH.md) instead of gating. Test count in particular must not gate, because deleting legacy tests after their replacements exist is a required outcome. Release APK size needs a full release build and is schedule-measured. | CODE_COMPLETE |
