...
| DATA-001I | Production parity matrix and migration rehearsal fixture. | Agent | [Target data parity matrix](TARGET_DATA_PARITY.md) + `TargetMigrationRehearsalTest` + `TargetCanonicalSeriesReadRepositoryTest`; category, tracking, excluded-scanlator policy, target backup, confirmed-link aggregate projection, and download artifact contract covered; source lifecycle, production tracker cutover, download index/filesystem reconciliation, production backup bridge, source selection/navigation, and production cutover remain open | IN_PROGRESS |
