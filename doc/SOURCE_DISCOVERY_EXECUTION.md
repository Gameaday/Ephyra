...
19. `DATA-001I`: production parity matrix and migration rehearsal fixture. **In progress**; target category schema, source-neutral tracking, excluded-scanlator policy, target backup round-trip, confirmed-link aggregate projection, multi-series/source rehearsal, isolated source lifecycle persistence, and descriptor reconciliation now pass. Reconciliation uses real `NativeSourceRegistry` descriptors, preserves install/enabled state, rejects stale/inconsistent revisions, and reports missing registry entries without auto-uninstalling them. Legacy numeric source rows are not fabricated into lifecycle state, and credentials/permission grants remain outside Room and backup. The download artifact contract, pure verifier, and read-only UniFile probe adapter are implemented and tested. The adapter counts supported directory page files and CBZ entries, computes byte size from the resolved UniFile, and reports unsafe/unreadable/unsupported storage as invalid. It never mutates storage or queue state. A transactional index publication adapter and legacy read-only filesystem adoption remain open. Production source startup wiring, production tracker scheduling/adapters, production backup bridge, source selection/navigation, and cutover rehearsal remain open. See `doc/TARGET_DATA_PARITY.md`.
