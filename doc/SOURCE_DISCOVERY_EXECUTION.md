...
17. `DATA-001G`: isolated target link repository and Room execution contract. **Not started**; do not add canonical grouping to the current one-source projection without this boundary.
18. `DATA-001H`: target repository production cutover decision. **Not started**; requires isolated read/write, backup, rollback, and device evidence.
