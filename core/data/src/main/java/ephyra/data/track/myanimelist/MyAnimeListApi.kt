...
class MyAnimeListApi(
    ...
    private fun parseDate(isoDate: String): Long = TrackDateFormats.parseDate(isoDate) ?: 0L

    private fun convertToIsoDate(epochTime: Long): String? {
        if (epochTime == 0L) {
            return ""
        }
        val formatted = TrackDateFormats.formatDate(epochTime)
        if (formatted == null) {
            logcat(LogPriority.DEBUG) { "Failed to format epoch $epochTime to ISO date; returning null" }
        }
        return formatted
    }
    ...
    companion object {
        ...
        /**
         * PKCE verifier generated by `authUrl` and echoed back by `getAccessToken`. The login flow
         * is inherently global (the authorize URL is produced before the API instance that
         * exchanges the code), so this stays static — but it is written on the UI thread and read
         * on an IO thread, hence `@Volatile`.
         */
        @Volatile
