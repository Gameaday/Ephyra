...
internal class PatchedDefaultRegister : InjektRegistrar {
    ...
    internal data class LoggerInfo(
    ...
    override fun <O : Any, T : O> addAlias(
        ...
        // factories existing or not, and data type compatibility is checked in the InjektRegistrar default methods
    ...
    private fun assertLogger(expectedLoggerType: Type) {
        if (loggerFactory == null) {
            ...
            throw InjektionException(
                "Logger factories registered with Injekt indicate they return type ${loggerFactory!!.forWhatType} " +
                    "but current injekt target is expecting type $expectedLoggerType",
            )
        }
    }
